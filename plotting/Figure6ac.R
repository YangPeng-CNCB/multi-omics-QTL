...
ggsave("boxplot.png", p,
       width = 6, height = 6, dpi = 300, bg = "white")  # 调整图像的宽度和高度
...
{
# 找到 P 为空的行
na_rows <- which(is.na(merged_data$pvalue_1))

# 仅在有 NA 的情况下进行最近邻填充
if (length(na_rows) > 0) {
  for (i in na_rows) {
    # 获取当前行的 CHR 和 BP
    chr_i <- merged_data$chr[i]
    bp_i <- merged_data$pos[i]
    
    # 筛选相同染色体的 gene 数据
    gene_subset <- gene[CHR == chr_i]
    
    # 计算所有基因位点与当前 BP 之间的距离
    gene_subset[, dist := abs(BP - bp_i)]
    
    # 找到距离最近的 BP，并获取其 P 值
    nearest_P <- gene_subset[which.min(dist), P]
    
    # 填充 NA 值
    merged_data$pvalue_1[i] <- nearest_P
  }
}

merged_data$pvalue_1 <- ifelse(merged_data$marker == "rs145450963", 
                               merged_data$pvalue_1, 
                               ifelse(merged_data$pvalue_1 == 1.7034e-141, NA, merged_data$pvalue_1))
...
p
library(ggplot2)
ggsave("C:/Users/yp/Desktop/合并写文章/绘图/crc-ugt2b17-dca.png", 
