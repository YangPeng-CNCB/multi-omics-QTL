...
for (i in 1:nrow(pairs)) {
  ...
  # 从input数据中提取相应的个体数据
  # 从input数据中提取相应的个体数据
  expr <- input[, m_id]  # 基因型数据（暴露）
  methy <- input[, cpg]  # 甲基化数据（中介）
  geno <- input[, snp]  # 基因表达数据（结果）
  
  valid_data <- complete.cases(expr, methy, geno)  # 仅保留无缺失值的行
  expr <- expr[valid_data]
  methy <- methy[valid_data]
  geno <- geno[valid_data]
  
  # 第一步：拟合暴露（SNP）对中介（甲基化/基因表达）的回归
  mod_mediator <- lm(methy ~ geno)  # M = μ2 + β2X + ε2
  
  # 第二步：拟合暴露和中介对结果的联合影响的回归
  mod_outcome <- lm(expr ~ geno + methy)  # Y = μ1 + β3X + β1M + ε1
  ...
  # 第一步：拟合暴露（SNP）对中介（甲基化/基因表达）的回归
  mod_mediator <- lm(expr ~ geno)  # M = μ2 + β2X + ε2
  
  # 第二步：拟合暴露和中介对结果的联合影响的回归
  mod_outcome <- lm(methy ~ geno + expr)  # Y = μ1 + β3X + β1M + ε1
